...
import infrastructure_manager, util
...
def get_printers():
    res = infrastructure_manager.list_containers("octoprint")  # return only octoprint related containers
...
def get_printer_status():
    ...
    if container_name:
        res = infrastructure_manager.gather_container_stats(container_name)
        return jsonify({"message": res, "type": "success"})
    else:
...
def deactivate_printer():
    container_name = request.values.get('printerid')
    if container_name:
        infrastructure_manager.stop_container(container_name)
        res = infrastructure_manager.list_running_containers(container_name)
        if len(res) == 0:
...
def start_printer():
    container_name = request.values.get('printerid')
    if container_name:
        infrastructure_manager.start_container(container_name)
        res = infrastructure_manager.list_running_containers(container_name)
        if len(res) == 1:
...
def start_all_printers():
    ...
    if number.isdigit():
        infrastructure_manager.start_printers(number)
...
def stop_all_printers():
    infrastructure_manager.stop_printers()
    return jsonify({"message": "true","type":"success"})
