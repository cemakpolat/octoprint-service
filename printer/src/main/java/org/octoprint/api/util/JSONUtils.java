...
public class JSONUtils {
	...
	public static <V> Map<String,V> loadMap(JsonObject jMap, Class<V> value) throws DeserializationException{
		...
		while(iter.hasNext())
		{
			...
			{
				result.put(aKey.toString(),(V) org.octoprint.api.util.JSONUtils.createObject(((JsonObject)jMap.get(aKey)).toJson(),value.getName()));
			}
			else
			{
				result.put(aKey.toString(),(V) createPrimitive(((String)jMap.get(aKey)),value.getName()));
			}
		}
	...
	public static <T> List<T> loadList(JsonArray jArray, Class<T> type) throws DeserializationException{
		...
		for(int count = 0; count < jArray.size(); count ++)
		{
			...
			{
				result.add((T) org.octoprint.api.util.JSONUtils.createObject(((JsonObject)jArray.get(count)).toJson(),type.getName()));
			}
			else
			{
				result.add((T) createPrimitive(((String)jArray.get(count)),type.getName()));
			}
	...
	public static <T> T createObject(String json, String type) throws DeserializationException{
		return createObject((JsonObject)Jsoner.deserialize(json), type);
	}
