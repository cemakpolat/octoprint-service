...
public class ConnectionCommand extends OctoPrintCommand {
	...
	public boolean connectWithPortName(final String portName) {
		OctoPrintHttpRequest request = this.createRequest();

		request.setType("POST");
		request.addParam("command", "connect");
		request.addParam("port", portName);

		return g_comm.executeUpdate(request);		
	}

	public boolean connectWithVirtualPort() {
		return connectWithPortName("VIRTUAL");
	}
