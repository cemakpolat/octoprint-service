...
public final class OctoPrintFolder extends OctoPrintFileInformation {
	...
	private void buildChildren(){
		...
		for(int count = 0; count < children.size(); count ++)
		{
			...
			if(t == FileType.FOLDER)
			{
				...
				m_children.add(new org.octoprint.api.model.OctoPrintFolder(t,aChild));
			}
			else
			{
	 ...
	 */
	public List<org.octoprint.api.model.OctoPrintFolder> getFolders(){
		List<org.octoprint.api.model.OctoPrintFolder> result = new ArrayList<org.octoprint.api.model.OctoPrintFolder>();
		...
		while(iter.hasNext())
		{
			...
			if(aFile.getType() == FileType.FOLDER)
			{
				result.add((org.octoprint.api.model.OctoPrintFolder)aFile);
			}
		}
